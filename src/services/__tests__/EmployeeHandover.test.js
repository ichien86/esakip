...
import service from '../EmployeeService';
...
describe('EmployeeService Plt Handover', () => {
  ...
  it('should trigger handover when new definitive leader is assigned', async () => {
    ...
    Employee.updateMany = vi.fn().mockResolvedValue({ nModified: 1 });
    CascadingAnnual.updateMany = vi.fn().mockResolvedValue({ nModified: 1 });
    PerjakinDocument.updateMany = vi.fn().mockResolvedValue({ nModified: 1 });
